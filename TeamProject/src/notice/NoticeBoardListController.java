...
public class NoticeBoardListController extends HttpServlet {
	...
	protected void doPro(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			//  게시판 전체 글의 개수 
			// 디비 객체 생성 NoticeBoardDAO bdao
			NoticeBoardDAO bdao = new NoticeBoardDAO();			
			// int count =  메서드호출  getNoticeCount()   count(*)
			...
			// 공지사항에 해당하는 공간 정보들을 가져온다.
			// noticeList=메서드호출  getBoardList(시작행startRow,몇개pageSize)
			if(count!=0){
			...
			request.setAttribute("noticedList", noticeList); // List -> Object 저장
